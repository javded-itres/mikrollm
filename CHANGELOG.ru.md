...
- Hub-alias **`auto`** может идти в пул чат-моделей. Хаб выбирает быструю / среднюю / сильную по тексту запроса; заголовок `X-MikroLLM-Routed-Model` называет узел и alias, которые ответили. Картинки и видео остаются на своих alias. [docs/ru/hub.md](docs/ru/hub.md)
...
- Админский чат: очередь — выезжающая панель сверху (стрелка). Видео/картинки остаются на своей вкладке после переключения.
- «Генерация» в чате — промпты текущей сессии, не шаги шлюза. GET статуса/файла видео не тратит RPM ключа.

## 0.0.7 — 2026-09-21

- Таймаут ожидания очереди больше не отменяет уже идущую генерацию (`context deadline exceeded` на длинном чате/видео).
- Слоты узла hub: чат / изображения / видео (по умолчанию 4 / 2 / 1) на Статусе; лишние запросы ждут в очереди сети.
- Вкладки в админском чате; в живой очереди видно, какой запрос сейчас генерируется.
- **Swagger UI** (светлая тема) на `/docs` и `/admin/docs`. Спека: `GET /openapi.json`. Authorize ключом `sk-`, чтобы дергать chat / images / videos.
