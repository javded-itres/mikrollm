...
Не выставляйте `:4000` в интернет без нужды. Админка защищена паролем, API — ключами, но это не замена VPN.
...
Лог контейнера (если `logging=yes`) попадает в `/log` RouterOS.
