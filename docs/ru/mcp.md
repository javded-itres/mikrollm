...
Агент (Grok, Cursor, Claude) может смотреть статус, настраивать провайдеры, модели, очереди и ключи, читать логи. `save_model.prompt_cache` = `inherit|off|auto|on`. `list_logs` / `log_stats` отдают `cached_tokens` и `saved_usd`.
...
url = "http://192.168.88.1:4000/mcp"   # или https://… после включения TLS, см. tls.md
enabled = true
...
| `refresh_health` | полный пересъём каталогов без кэша (OpenRouter: chat + image + video) + сводка |
| `refresh_provider` | то же для одного бэкенда по `id` |
| `list_providers` / `upsert_provider` / `delete_provider` | Ollama, vLLM, LM Studio, OpenRouter, Ollama Cloud. Токен маскируется; пустой `token` не затирает ключ. `enabled` и `weight` без поля не меняются. Выключить: `id` + `enabled: false` |
...
| `list_policies` / `list_plugins` / `save_policy` / `delete_policy` | фильтры; `kind=nsfw` или `plugins: ["nsfw","adult"]`. Цели alias/queue/model, один id — один раз |
