...
[English](../install-mikrotik.md) · **Русский**
...
HTTPS на том же `:4000` (без второго контейнера): [tls.md](tls.md). Self-signed — `MIKROLLM_TLS_AUTO=1`. Let's Encrypt — `MIKROLLM_ACME_HOSTS` и dst-nat WAN **только 80** на контейнер; имя в DNS, с LAN — static DNS на `192.168.88.1`.
