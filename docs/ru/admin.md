...
- **Обновить модели** на карточке — принудительно скачать каталог с этого провайдера (без кэша). OpenRouter: `/models` + video (`/models?output_modalities=video`, `/videos/models`).
...
- Изображение: `/v1/images/generations` (OpenRouter — через chat + `modalities`).
- Видео: `/v1/videos`, статус опрашивается сам.
- Модели с image/video помечены пилюлями в каталоге и в селекторе. API-ключ не нужен — только сессия админки.
