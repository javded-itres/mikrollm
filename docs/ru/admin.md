...
На **Статусе** у узла hub: сколько запросов брать сразу (чат / изображения / видео, по умолчанию 4 / 2 / 1). Остальные ждут в очереди сети.
...
2. **RAM**: `82 · в RAM` загружает веса (Ollama `keep_alive: -1`, LM Studio `/api/v1/models/load`), `выгрузить` снимает. Большая модель может грузиться несколько минут. Ollama грузит с наибольшим `num_ctx` из профилей «Параметры» alias на эту модель (иначе — Контекст alias): дефолт Ollama — 4096, а num_ctx в запросе заставляет перегружать веса, поэтому задайте контекст в Параметрах и нажмите «в RAM» ещё раз. У vLLM кнопок нет: модель задана процессом `vllm serve`.
