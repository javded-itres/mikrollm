...
Пока галочка включена, в шлюзе живёт зарезервированный alias **`auto`**. Оператор хаба (**Оператор → Настройки**) выбирает узел и модель по умолчанию; MikroLLM копирует это в `auto` при каждом обновлении каталога (~15 с). В чате и `/v1` можно звать `model: "auto"`. Вышли из сети (или оператор сбросил настройку) — `auto` пропадает.
